...
func (l *Light) UpdateDimmingSchedulesConfigToDevice(dev *models.Device) error {
	deviceName := dev.Name
	...
	return l.WriteDimmingScheduleToDevice(deviceName, request, schs, DimmingScheduleLimit)
}

func (l *Light) WriteDimmingScheduleToDevice(deviceName string, cmReq *sdkModel.CommandRequest, schs []appModels.EdgeDimmingSchedule, dimmingScheduleLimit int) error {
...
func (l *Light) SyncDimmingScheduleDBByGroups(deviceName string, groups []string) (string, bool) {
	...
	var change = false
	schedules := appModels.DimmingScheduleGetFromDB(&dev)
	if len(schedules) <= 0 {
